    ...
    toTarget() {
        if (!this.memoizedTarget) {
            if (this.limitType === "F" /* First */) {
                this.memoizedTarget = newTarget(this.path, this.collectionGroup, this.orderBy, this.filters, this.limit, this.startAt, this.endAt);
            }
            else {
                ...
                this.memoizedTarget = newTarget(this.path, this.collectionGroup, orderBys, this.filters, this.limit, startAt, endAt);
            }
        }
...
function queryEquals(left, right) {
    return (targetEquals(left.toTarget(), right.toTarget()) &&
...
function stringifyQuery(query) {
    return `Query(target=${stringifyTarget(query.toTarget())}; limitType=${query.limitType})`;
}
...
function canonifyFilter(filter) {
    debugAssert(filter instanceof FieldFilter, 'canonifyFilter() only supports FieldFilters');
...
function filterEquals(f1, f2) {
    return (f1 instanceof FieldFilter &&
...
function stringifyFilter(filter) {
    debugAssert(filter instanceof FieldFilter, 'stringifyFilter() only supports FieldFilters');
...
function sortsBeforeDocument(bound, orderBy, doc) {
    debugAssert(bound.position.length <= orderBy.length, "Bound has more components than query's orderBy");
    let comparison = 0;
...
function fromName(serializer, name) {
    ...
    hardAssert(resource.get(1) === serializer.databaseId.projectId, 'Tried to deserialize key from different project: ' +
        resource.get(1) +
        ' vs ' +
        serializer.databaseId.projectId);
    hardAssert((!resource.get(3) && !serializer.databaseId.database) ||
        resource.get(3) === serializer.databaseId.database, 'Tried to deserialize key from different database: ' +
        resource.get(3) +
        ' vs ' +
...
function applyTransformOperationToLocalView(transform, previousValue, localWriteTime) {
    if (transform instanceof ServerTimestampTransform) {
        ...
        debugAssert(transform instanceof NumericIncrementTransformOperation, 'Expected NumericIncrementTransformOperation but was: ' + transform);
...
function applyTransformOperationToRemoteDocument(transform, previousValue, transformResult) {
    ...
    debugAssert(transformResult !== null, "Didn't receive transformResult for non-array transform");
...
function applyMutationToRemoteDocument(mutation, maybeDoc, mutationResult) {
    ...
    if (mutation instanceof SetMutation) {
        ...
        debugAssert(mutation instanceof DeleteMutation, 'Unexpected mutation type: ' + mutation);
...
function verifyMutationKeyMatches(mutation, maybeDoc) {
    if (maybeDoc != null) {
        debugAssert(maybeDoc.key.isEqual(mutation.key), 'Can only apply a mutation to a document with the same key');
    }
}
/**
...
function requireDocument(mutation, maybeDoc) {
    ...
    debugAssert(maybeDoc.key.isEqual(mutation.key), 'Can only transform a document with the same key');
    return maybeDoc;
}
/**
...
function serverTransformResults(fieldTransforms, baseDoc, serverTransformResults) {
    ...
    hardAssert(fieldTransforms.length === serverTransformResults.length, `server transform result count (${serverTransformResults.length}) ` +
...
function transformObject(mutation, data, transformResults) {
    debugAssert(transformResults.length === mutation.fieldTransforms.length, 'TransformResults length mismatch.');
    const builder = new ObjectValueBuilder(data);
...
function applyDeleteMutationToLocalView(mutation, maybeDoc) {
    ...
    if (maybeDoc) {
        debugAssert(maybeDoc.key.isEqual(mutation.key), 'Can only apply mutation to document with same key');
    }
...
class IndexedDbPersistence {
    ...
    async shutdown() {
        ...
        await this.runTransaction('shutdown', 'readwrite', txn => {
            ...
            logDebug(LOG_TAG$3, 'Proceeding with shutdown despite failure: ', e);
        });
        this.simpleDb.close();
...
class SimpleDb {
    ...
    static openOrCreate(name, version, schemaConverter) {
        ...
        logDebug(LOG_TAG$4, 'Opening database:', name);
        return new PersistencePromise((resolve, reject) => {
            ...
            request.onupgradeneeded = (event) => {
                logDebug(LOG_TAG$4, 'Database "' + name + '" requires upgrade from version:', event.oldVersion);
                const db = event.target.result;
                schemaConverter
                    .createOrUpgrade(db, request.transaction, event.oldVersion, SCHEMA_VERSION)
                    .next(() => {
                    logDebug(LOG_TAG$4, 'Database upgrade to version ' + SCHEMA_VERSION + ' complete');
                });
            };
        }).toPromise();
    }
    ...
    static delete(name) {
        logDebug(LOG_TAG$4, 'Removing database:', name);
    ...
    async runTransaction(mode, objectStores, transactionFn) {
        ...
        while (true) {
            ...
            try {
                ...
                logDebug(LOG_TAG$4, 'Transaction failed with error: %s. Retrying: %s.', error.message, retryable);
                if (!retryable) {
...
class SimpleDbTransaction {
    ...
    abort(error) {
        ...
        if (!this.aborted) {
            logDebug(LOG_TAG$4, 'Aborting transaction:', error ? error.message : 'Client-initiated abort');
            this.aborted = true;
...
class SimpleDbStore {
    ...
    put(keyOrValue, value) {
        let request;
        if (value !== undefined) {
            logDebug(LOG_TAG$4, 'PUT', this.store.name, keyOrValue, value);
            request = this.store.put(value, keyOrValue);
        }
        else {
            logDebug(LOG_TAG$4, 'PUT', this.store.name, '<auto-key>', keyOrValue);
     ...
     */
    add(value) {
        logDebug(LOG_TAG$4, 'ADD', this.store.name, value, value);
    ...
    get(key) {
        ...
        return wrapRequest(request).next(result => {
            ...
            logDebug(LOG_TAG$4, 'GET', this.store.name, key, result);
            return result;
        });
    }
    delete(key) {
        logDebug(LOG_TAG$4, 'DELETE', this.store.name, key);
     ...
     */
    count() {
        logDebug(LOG_TAG$4, 'COUNT', this.store.name);
    ...
    deleteAll(indexOrRange, range) {
        logDebug(LOG_TAG$4, 'DELETE ALL', this.store.name);
...
function newMultiTabLocalStore(
/** Manages our in-memory or durable persistence. */
persistence, queryEngine, initialUser) {
...
class PersistentWriteStream extends PersistentStream {
    ...
    writeHandshake() {
        ...
        debugAssert(!this.lastStreamToken, 'Stream token should be empty during handshake');
    ...
    writeMutations(mutations) {
        ...
        debugAssert(!!this.lastStreamToken, 'Trying to write mutation without a token');
        const request = {
...
class SyncEngineImpl {
    ...
    async unlisten(query) {
        ...
        const queryView = this.queryViewsByQuery.get(query);
        debugAssert(!!queryView, 'Trying to unlisten on query not found:' + stringifyQuery(query));
    ...
    getRemoteKeysForTarget(targetId) {
        ...
        if (limboResolution && limboResolution.receivedDocument) {
            ...
            for (const query of queries) {
                const queryView = this.queryViewsByQuery.get(query);
                debugAssert(!!queryView, `No query view found for ${stringifyQuery(query)}`);
...
// PORTING NOTE: Manages state synchronization in multi-tab environments.
sharedClientState, currentUser, maxConcurrentLimboResolutions) {
    return new SyncEngineImpl(localStore, remoteStore, datastore, sharedClientState, currentUser, maxConcurrentLimboResolutions);
}
/**
...
class MultiTabSyncEngineImpl extends SyncEngineImpl {
    ...
    async applyBatchState(batchId, batchState, error) {
        ...
        if (documents === null) {
            ...
            // as applicable.
            logDebug(LOG_TAG$9, 'Cannot apply mutation batch with id: ' + batchId);
            return;
        }
    ...
    async applyTargetState(targetId, state, error) {
        if (this._isPrimaryClient) {
            ...
            logDebug(LOG_TAG$9, 'Ignoring unexpected query state notification.');
            return;
        }
        if (this.queriesByTarget.has(targetId)) {
            switch (state) {
    ...
    async applyActiveTargetsChange(added, removed) {
        ...
        for (const targetId of added) {
            if (this.queriesByTarget.has(targetId)) {
                ...
                logDebug(LOG_TAG$9, 'Adding an already active target ' + targetId);
                continue;
            }
            const target = await this.localStore.getTarget(targetId);
...
function newMultiTabSyncEngine(localStore, remoteStore, datastore, sharedClientState, currentUser, maxConcurrentLimboResolutions) {
    return new MultiTabSyncEngineImpl(localStore, remoteStore, datastore, sharedClientState, currentUser, maxConcurrentLimboResolutions);
}
...
class RemoteStore {
    ...
    async disableNetworkUntilRecovery(e, op) {
        if (isIndexedDbTransactionError(e)) {
            debugAssert(!this.offlineCauses.has(1 /* IndexedDbFailed */), 'Unexpected network event when IndexedDB was marked failed.');
...
class MemoryComponentProvider {
    ...
    createRemoteStore(cfg) {
        return new RemoteStore(this.localStore, cfg.datastore, cfg.asyncQueue, onlineState => this.syncEngine.applyOnlineStateChange(onlineState, 0 /* RemoteStore */), newConnectivityMonitor());
    }
    createSharedClientState(cfg) {
        return new MemorySharedClientState();
    }
    createSyncEngine(cfg) {
        return newSyncEngine(this.localStore, this.remoteStore, cfg.datastore, this.sharedClientState, cfg.initialUser, cfg.maxConcurrentLimboResolutions);
    }
...
class IndexedDbComponentProvider extends MemoryComponentProvider {
    ...
    createLocalStore(cfg) {
        return newMultiTabLocalStore(this.persistence, new IndexFreeQueryEngine(), cfg.initialUser);
    }
    createSyncEngine(cfg) {
        const syncEngine = newMultiTabSyncEngine(this.localStore, this.remoteStore, cfg.datastore, this.sharedClientState, cfg.initialUser, cfg.maxConcurrentLimboResolutions);
...
class FirestoreClient {
     ...
     */
    start(databaseInfo, componentProvider, persistenceSettings) {
        this.verifyNotTerminated();
        this.databaseInfo = databaseInfo;
...
class Firestore {
    ...
    onSnapshotsInSync(arg) {
        ...
        if (isPartialObserver(arg)) {
            return addSnapshotsInSyncListener(this._firestoreClient, arg);
        }
        else {
    ...
    configureClient(componentProvider, persistenceSettings) {
        ...
        return this._firestoreClient.start(databaseInfo, componentProvider, persistenceSettings);
    }
    ...
    collectionGroup(collectionId) {
        ...
        this.ensureClientConfigured();
        return new Query$1(new Query(ResourcePath.emptyPath(), collectionId), this, 
        /* converter= */ null);
    }
...
function addSnapshotsInSyncListener(firestoreClient, observer) {
...
class DocumentReference extends DocumentKeyReference {
    ...
    onSnapshot(...args) {
        ...
        return addDocSnapshotListener(this._firestoreClient, this._key, internalOptions, observer);
    }
    get(options) {
        ...
        if (options && options.source === 'cache') {
            ...
            return getDocViaSnapshotListener(this._firestoreClient, this._key, options).then(snapshot => this._convertToDocSnapshot(snapshot));
        }
    }
...
function addDocSnapshotListener(firestoreClient, key, options, observer) {
    let errHandler = (err) => {
    ...
    const internalListener = firestoreClient.listen(Query.atPath(key.path), asyncObserver, options);
    return () => {
        asyncObserver.mute();
 ...
 * SnapshotListener.
 */
function getDocViaSnapshotListener(firestoreClient, key, options) {
    const result = new Deferred();
    const unlisten = addDocSnapshotListener(firestoreClient, key, {
...
class BaseQuery {
    ...
    createFilter(fieldPath, op, value) {
        ...
        if (fieldPath.isKeyField()) {
            ...
            fieldValue = parseQueryValue(this._dataReader, 'Query.where', value, op === "in" /* IN */);
        }
...
class Query$1 extends BaseQuery {
    ...
    onSnapshot(...args) {
        ...
        return addQuerySnapshotListener(firestoreClient, this._query, options, observer);
    }
    get(options) {
        ...
        return (options && options.source === 'cache'
            ...
            : getDocsViaSnapshotListener(firestoreClient, this._query, options)).then(snap => new QuerySnapshot(this.firestore, this._query, snap, this._converter));
    }
}
/**
 ...
 * SnapshotListener.
 */
function getDocsViaSnapshotListener(firestore, query, options) {
    const result = new Deferred();
    const unlisten = addQuerySnapshotListener(firestore, query, {
        includeMetadataChanges: true,
        waitForSyncWhenOnline: true
    }, {
        next: snapshot => {
...
function addQuerySnapshotListener(firestore, query, options, observer) {
    let errHandler = (err) => {
        console.error('Uncaught Error in onSnapshot:', err);
    };
    if (observer.error) {
...
function changesFromSnapshot(snapshot, includeMetadataChanges, converter) {
    if (snapshot.oldDocs.isEmpty()) {
        ...
        return snapshot.docChanges.map(change => {
            ...
            debugAssert(!lastDoc || newQueryComparator(snapshot.query)(lastDoc, change.doc) < 0, 'Got added events in wrong order');
            lastDoc = change.doc;
            return {
                type: 'added',
                doc,
